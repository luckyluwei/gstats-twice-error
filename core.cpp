  ...
  fpTagBypass(0)
{
  ...
  if (coredynp.core_ty==Inorder)
    {
      int_bypass   = new interconnect("Int Bypass Data", 1, 1, int(ceil(XML->sys.machine_bits/32.0)*32), rfu->int_regfile_height + exeu->FU_height + lsq_height, &interface_ip, 3);
      intTagBypass = new interconnect("Int Bypass tag" , 1, 1, coredynp.perThreadState, rfu->int_regfile_height + exeu->FU_height + lsq_height + scheu->Iw_height, &interface_ip, 3);
      fp_bypass    = new interconnect("FP Bypass Data" , 1, 1, int(ceil(XML->sys.machine_bits/32.0)*32*1.5), rfu->fp_regfile_height + fp_u->FU_height, &interface_ip, 3);
      fpTagBypass  = new interconnect("FP Bypass tag"  , 1, 1, coredynp.perThreadState, rfu->fp_regfile_height + fp_u->FU_height + lsq_height + scheu->Iw_height, &interface_ip, 3);
    }
  else
    {//OOO
      ...
      if (coredynp.scheu_ty==PhysicalRegFile)
        {
          ...
          fpTagBypass  = new interconnect("FP Bypass tag"  , 1, 1, coredynp.phy_freg_width,
...
void BranchPredictor::displayEnergy(uint32_t indent,int plevel,bool is_tdp)
{
  ...
  if (is_tdp)
    ...
    {
      //		cout << indent_str_next << "Global Predictor    TDP Dynamic = " << globalBPT->rt_power.readOp.dynamic*clockRate << " W" << endl;
      ...
      //		cout << indent_str_next << "Local Predictor   TDP Dynamic = " << L1_localBPT->rt_power.readOp.dynamic*clockRate  << " W" << endl;
      ...
      //		cout << indent_str_next << "Chooser   TDP Dynamic = " << chooser->rt_power.readOp.dynamic*clockRate  << " W" << endl;
...
void InstFetchU::computeEnergy(bool is_tdp)
{
  ...
  if (is_tdp)
  ...
  else
    {
      ...
      IB->stats_t.readAc.access = IB->stats_t.writeAc.access = XML->sys.core[ithCore].total_instructions/XML->sys.core[ithCore].issue_width;
  ...
  icachecc.power_t.readOp.dynamic	+=  icache.missb->stats_t.readAc.access*icache.missb->local_result.power.searchOp.dynamic +
...
void InstFetchU::displayEnergy(uint32_t indent,int plevel,bool is_tdp)
{
  ...
  if (is_tdp)
    ...
    {
      //		cout << indent_str_next << "Instruction Cache    TDP Dynamic = " << icache.rt_power.readOp.dynamic*clockRate << " W" << endl;
      ...
      //		cout << indent_str_next << "Instruction Buffer   TDP Dynamic = " << IB->rt_power.readOp.dynamic*clockRate  << " W" << endl;
      ...
      //		cout << indent_str_next << "Branch Target Buffer   TDP Dynamic = " << BTB->rt_power.readOp.dynamic*clockRate  << " W" << endl;
      ...
      //		cout << indent_str_next << "Branch Predictor   TDP Dynamic = " << BPT->rt_power.readOp.dynamic*clockRate  << " W" << endl;
          ...
          if (coredynp.scheu_ty==PhysicalRegFile)
            {
              ...
              power	       =  power + (iFRAT->power_t + fFRAT->power_t)
                + (iRRAT->power_t + fRRAT->power_t)
                + (ifreeL->power_t + ffreeL->power_t);
            }
          else if (coredynp.scheu_ty==ReservationStation)
            {
              iFRAT->power   =  iFRAT->power_t + (iFRAT->local_result.power ) * pppm_lkg + idcl->power_t;
...
void RENAMINGU::displayEnergy(uint32_t indent,int plevel,bool is_tdp)
{
  ...
  if (is_tdp)
  ...
  else
    {
      if (coredynp.core_ty==OOO)
        {
          cout << indent_str_next << "Int Front End RAT    TDP Dynamic = " << iFRAT->rt_power.readOp.dynamic*clockRate << " W" << endl;
  ...
  if (is_tdp)
    ...
    {//rtp
      ...
      if (coredynp.core_ty==OOO)
        ...
        {
          int_inst_window->stats_t.readAc.access  = 2*XML->sys.core[ithCore].int_instructions + 2*XML->sys.core[ithCore].fp_instructions;
  ...
  if (coredynp.core_ty==OOO)
    {
      ...
      //ROB->power.reset();
      int_inst_window->power_t.readOp.dynamic  +=  int_inst_window->local_result.power.searchOp.dynamic * int_inst_window->stats_t.readAc.access +
      ...
      fp_inst_window->power_t.readOp.dynamic  +=  fp_inst_window->local_result.power.searchOp.dynamic * fp_inst_window->stats_t.readAc.access +
...
void SchedulerU::displayEnergy(uint32_t indent,int plevel,bool is_tdp)
{
  ...
  if (is_tdp)
  ...
  else
    {
      if (coredynp.core_ty==OOO)
        {
          cout << indent_str_next << "Instruction Window    TDP Dynamic = " << int_inst_window->rt_power.readOp.dynamic*clockRate << " W" << endl;
          ...
          cout << indent_str_next << "FP Instruction Window   TDP Dynamic = " << fp_inst_window->rt_power.readOp.dynamic*clockRate  << " W" << endl;
...
void LoadStoreU::computeEnergy(bool is_tdp)
{
  ...
  else
    {
      ...
      if (XML->sys.core[ithCore].scoore)

        {
          ...
          vpc_buffer->stats_t.readAc.access  = XML->sys.core[ithCore].vpc_buffer.read_hits + XML->sys.core[ithCore].vpc_buffer.read_misses;
  ...
  //LSQ->power.reset();
  if (XML->sys.core[ithCore].scoore){
  dcache.power_t.readOp.dynamic	+= (dcache.caches->stats_t.readAc.access*dcache.caches->local_result.power.readOp.dynamic+
  ...
  dcachecc.power_t.readOp.dynamic	=  (dcache.missb->stats_t.readAc.access*dcache.missb->local_result.power.searchOp.dynamic +
  ...
  dcachecc.power_t.readOp.dynamic	+=  (dcache.prefetchb->stats_t.readAc.access*dcache.prefetchb->local_result.power.searchOp.dynamic +
  ...
  if (XML->sys.core[ithCore].scoore)
    {
      if(XML->sys.core[ithCore].VPCfilter.dcache_config[0]>0){
        VPCfilter.power_t.readOp.dynamic	+= (VPCfilter.caches->stats_t.readAc.access*VPCfilter.caches->local_result.power.readOp.dynamic+
                                                    ...
                                                    VPCfilter.caches->stats_t.writeAc.access*VPCfilter.caches->local_result.power.writeOp.dynamic);//write miss will generate a write later

        VPCfilter.power_t.readOp.dynamic	+=  VPCfilter.missb->stats_t.readAc.access*VPCfilter.missb->local_result.power.searchOp.dynamic +
                                                    VPCfilter.missb->stats_t.writeAc.access*VPCfilter.missb->local_result.power.writeOp.dynamic;//each access to missb involves a CAM and a write

        VPCfilter.power_t.readOp.dynamic	+=  VPCfilter.ifb->stats_t.readAc.access*VPCfilter.ifb->local_result.power.searchOp.dynamic +
                                                    VPCfilter.ifb->stats_t.writeAc.access*VPCfilter.ifb->local_result.power.writeOp.dynamic;

        VPCfilter.power_t.readOp.dynamic	+=  VPCfilter.prefetchb->stats_t.readAc.access*VPCfilter.prefetchb->local_result.power.searchOp.dynamic +
                                                    VPCfilter.prefetchb->stats_t.writeAc.access*VPCfilter.prefetchb->local_result.power.writeOp.dynamic;

        VPCfilter.power_t.readOp.dynamic	+=  VPCfilter.wbb->stats_t.readAc.access*VPCfilter.wbb->local_result.power.searchOp.dynamic +
                                                    VPCfilter.wbb->stats_t.writeAc.access*VPCfilter.wbb->local_result.power.writeOp.dynamic;
      }
      vpc_buffer->power_t.readOp.dynamic  +=  vpc_buffer->stats_t.readAc.access*(vpc_buffer->local_result.power.searchOp.dynamic + vpc_buffer->local_result.power.readOp.dynamic) ;
      //strb->power_t.readOp.dynamic  +=  strb->stats_t.readAc.access*(strb->local_result.power.searchOp.dynamic + strb->local_result.power.readOp.dynamic) +
      //strb->stats_t.writeAc.access*strb->local_result.power.writeOp.dynamic;//every memory access invloves at least two operations on strb
      //ldrb->power_t.readOp.dynamic  +=  ldrb->stats_t.readAc.access*(ldrb->local_result.power.searchOp.dynamic + ldrb->local_result.power.readOp.dynamic) +
      //ldrb->stats_t.writeAc.access*ldrb->local_result.power.writeOp.dynamic;//every memory access invloves at least two operations on ldrb
    }else
    {
      ...
      if (coredynp.core_ty==OOO)
        {
          ...
          //eka, may need to check the first call 
          //ssit->power.reset();
          ssit->power_t.readOp.dynamic  +=  ssit->stats_t.readAc.access*(ssit->local_result.power.readOp.dynamic)+
            ssit->stats_t.writeAc.access*ssit->local_result.power.writeOp.dynamic;//every memory access invloves at least two operations on LoadQ

          lfst->power_t.reset();
          //eka, may need to check the first call 
          //lfst->power.reset();
          lfst->power_t.readOp.dynamic  +=  lfst->stats_t.readAc.access*(lfst->local_result.power.readOp.dynamic)+
            lfst->stats_t.writeAc.access*lfst->local_result.power.writeOp.dynamic;//every memory access invloves at least two operations on LoadQ
        }
    }
  if (is_tdp)
    {
      dcache.power = dcache.power_t + (dcache.caches->local_result.power)*pppm_lkg;
      dcachecc.power = dcachecc.power_t + (dcache.missb->local_result.power +
      ...
      if (XML->sys.core[ithCore].scoore)
        {
          if(XML->sys.core[ithCore].VPCfilter.dcache_config[0]>0){
            VPCfilter.power = VPCfilter.power_t + (VPCfilter.caches->local_result.power)*pppm_lkg +
              (VPCfilter.missb->local_result.power +
               VPCfilter.ifb->local_result.power +
               //VPCfilter.prefetchb->local_result.power +
               VPCfilter.wbb->local_result.power)*pppm_Isub;
          }
  ...
  else
    {
      ...
      dcachecc.rt_power = dcachecc.power_t + (
      ...
      if (XML->sys.core[ithCore].scoore)
        {
          if(XML->sys.core[ithCore].VPCfilter.dcache_config[0]>0){
            VPCfilter.rt_power = VPCfilter.power_t + (VPCfilter.caches->local_result.power +
                                                      VPCfilter.missb->local_result.power +
                                                      VPCfilter.ifb->local_result.power +     
                                                      //VPCfilter.prefetchb->local_result.power +
                                                      VPCfilter.wbb->local_result.power)*pppm_lkg;
...

        {
          ...
          if (XML->sys.core[ithCore].scoore)
            ...
            }else
            {
              ...
              cout << indent_str_next << "TDP Dynamic = " << (ssit->power.readOp.dynamic+lfst->power.readOp.dynamic)*clockRate  << " W" << endl;
              cout << indent_str_next << "Subthreshold Leakage = " << ssit->power.readOp.leakage +  lfst->power.readOp.leakage << " W" << endl;
              cout << indent_str_next << "Gate Leakage = " << ssit->power.readOp.gate_leakage + lfst->power.readOp.gate_leakage   << " W" << endl;
              cout << indent_str_next << "Runtime Dynamic = " << ssit->rt_power.readOp.dynamic/executionTime + lfst->rt_power.readOp.dynamic/executionTime<< " W" << endl;
              cout << indent_str_next << "Dynamic per read access = " <<  
                ssit->local_result.power.readOp.dynamic +
                lfst->local_result.power.readOp.dynamic
                   << " W" << endl;
              cout <<endl;
            }
        }
  ...
  else
    {
      ...
      if (coredynp.core_ty==Inorder)
      ...
      else
        {
          ...
          cout << indent_str_next << "TDP Dynamic = " << (ssit->rt_power.readOp.dynamic+lfst->power.readOp.dynamic)*clockRate  << " W" << endl;
          cout << indent_str_next << "Subthreshold Leakage = " << ssit->rt_power.readOp.leakage +  lfst->rt_power.readOp.leakage << " W" << endl;
          cout << indent_str_next << "Gate Leakage = " << ssit->rt_power.readOp.gate_leakage + lfst->rt_power.readOp.gate_leakage   << " W" << endl;
        }
    }
  ...
  if (is_tdp)
    {
      ...
      cout << indent_str_next << "Runtime Dynamic = " << 
        dtlb->local_result.power.readOp.dynamic 
...
void EXECU::computeEnergy(bool is_tdp)
{
  ...
  if (is_tdp)
  ...
  else
    {
      ...
      exeu->rt_power.readOp.dynamic = exeu->power.readOp.dynamic * XML->sys.core[ithCore].int_instructions/ XML->sys.core[0].ALU_per_core;
      exeu->rt_power.readOp.leakage = exeu->local_result.power.readOp.leakage;
      fp_u->rt_power.readOp.dynamic = fp_u->power.readOp.dynamic * XML->sys.core[ithCore].fp_instructions/ XML->sys.core[0].FPU_per_core; 
...
void EXECU::displayEnergy(uint32_t indent,int plevel,bool is_tdp)
{
  ...
  if (is_tdp)
  ...
  else
    {
      ...
      cout << indent_str_next << "Instruction Sheduler   TDP Dynamic = " << scheu->rt_power.readOp.dynamic*clockRate  << " W" << endl;
      ...
      cout << indent_str_next << "Results Broadcast Bus   TDP Dynamic = " << bypass.rt_power.readOp.dynamic*clockRate  << " W" << endl;
...
// below is my code
int mypwrstate=0;
MyPwrModel = new PowerModel;
mypwrstate=Core::MyPwrModel->myParisDevMaxT();
switch(mypwrstate){
case 0:
case 1:
 coredynp.issueW = XML->sys.core[ithCore].issue_width;
	break;
case 2:
 coredynp.issueW = XML->sys.core[ithCore].issue_width-1;
	break;
default:
 coredynp.issueW = XML->sys.core[ithCore].issue_width;
}
...
void Core::update_rtparam(ParseXML *XML_interface, int ithCore_, 
                          InputParameter* interface_ip_, Core *core)
{
  ithCore = ithCore_;
...
void InstFetchU::buildInstCache() {
  ...
  interface_ip.cache_sz            = (XML->sys.core[ithCore].icache.buffer_sizes[0]*interface_ip.line_sz < 64? 64:XML->sys.core[ithCore].icache.buffer_sizes[0]*interface_ip.line_sz);
...
void InstFetchU::buildInstBuffer(){
  //Instruction buffer
  bool debug= false;
  int data							   = XML->sys.core[ithCore].instruction_length*XML->sys.core[ithCore].issue_width;//icache.caches.l_ip.line_sz; //multiple threads timing sharing the instruction buffer.
  ...
  interface_ip.cache_sz            = XML->sys.core[ithCore].instruction_buffer_size*interface_ip.line_sz>64?
                                     XML->sys.core[ithCore].instruction_buffer_size*interface_ip.line_sz:64;
...
void SchedulerU::buildInorderScheduler() {
  ...
  instruction_selection = new selection_logic(is_default, XML->sys.core[ithCore].instruction_window_size,
      XML->sys.core[ithCore].issue_width*XML->sys.core[ithCore].number_hardware_threads,
      &interface_ip);
}
...
void SchedulerU::buildIntInstWindow(){
  ...
  else
  {
    ...
    data  = int(ceil((coredynp.instruction_length+2*(coredynp.phy_ireg_width - coredynp.arch_ireg_width)+
            2*coredynp.int_data_width)/8.0));//TODO: need divided by 2? means only after both operands avaible, the whole data will be read out.
...
void SchedulerU::buildROB(){
  ...
  if(coredynp.scheu_ty==PhysicalRegFile)
  {
    ...
    data = int(ceil((robExtra+coredynp.pc_width +
            /*coredynp.instruction_length*/ 4 + 1*coredynp.phy_ireg_width)/8.0));
  }
  else
  {
...
void LoadStoreU::buildStoreSets() {
  ...
  interface_ip.pure_cam                       = false;
  interface_ip.pure_ram                       = true;
  interface_ip.line_sz                        = log2(XML->sys.core[ithCore].lfst_size)*XML->sys.core[ithCore].issue_width/2; // should be divided by BB4cycle
...
void LoadStoreU::buildLSQ() {
  int tag                              = XML->sys.core[ithCore].opcode_width+XML->sys.virtual_address_width +int(ceil(log2(XML->sys.core[ithCore].number_hardware_threads))) + EXTRA_TAG_BITS;
...
void MemManU::buildDTLB() {
  ...
  interface_ip.tag_w                          = tag;
  interface_ip.line_sz                        = int(ceil(data/8.0));                                                                                                                                                  // int(ceil(pow(2.0,ceil(log2(data)))/8.0));
  interface_ip.cache_sz                       = XML->sys.core[ithCore].dtlb.number_entries*interface_ip.line_sz*XML->sys.core[ithCore].number_hardware_threads;                                                       // TODO:fix bug in XML
  interface_ip.assoc                          = 0;
  interface_ip.nbanks                         = 1;
  interface_ip.out_w                          = interface_ip.line_sz*8;
  interface_ip.access_mode                    = 2;
...
void RENAMINGU::buildIFRATRam() {
  ...
  area.set_area(area.get_area()+ iFRAT->area.get_area()+ iFRAT->local_result.area*XML->sys.core[ithCore].number_hardware_threads);
}
...
void RENAMINGU::buildFFRATRam() {
  ...
  area.set_area(area.get_area()+ fFRAT->area.get_area()+ fFRAT->local_result.area*XML->sys.core[ithCore].number_hardware_threads);
}
...
void RENAMINGU::buildIRRAT() {
  ...
  area.set_area(area.get_area()+ iRRAT->area.get_area()+ iRRAT->local_result.area*XML->sys.core[ithCore].number_hardware_threads);
}
...
void RENAMINGU::buildFRRAT() {
  ...
  area.set_area(area.get_area()+ fRRAT->area.get_area()+ fRRAT->local_result.area*XML->sys.core[ithCore].number_hardware_threads);
}
...
void RENAMINGU::buildIFreeL() {
  ...
  area.set_area(area.get_area()+ ifreeL->area.get_area()+ ifreeL->local_result.area*XML->sys.core[ithCore].number_hardware_threads);
}
...
void RENAMINGU::buildFFreeL() {
  ...
  area.set_area(area.get_area()+ ffreeL->area.get_area()+ ffreeL->local_result.area*XML->sys.core[ithCore].number_hardware_threads);
}
